...
class Court_Case_Scraper:
    ...
    def __extract_fields(self, soup:Optional[BeautifulSoup]) -> None:
        ...
        if soup.select('div.table.table-responsive>table>tr>td') != []:
            party_data_list = soup.select('div.table.table-responsive>table>tr')
            for raw_tags_data in party_data_list[2:]:
    ...
    def __extract_data_from_paginattion(self, soup:Optional[BeautifulSoup]) -> None:
        ...
        while True:
            ...
            soup = BeautifulSoup(paginattion_response.content, 'lxml')
            if soup.select('div.table-responsive>table>table>tr>td') == []:
                paginattion_response_data_list = soup.select('div.table-responsive>table>tr')
                if paginattion_response_data_list != []:
                    for raw_tags_data in paginattion_response_data_list:
                        raw_tag_data_list = raw_tags_data.select('td')
                        if raw_tag_data_list != []:
                            sr_no = raw_tag_data_list[0].select('font')[0].get_text(strip=True).replace('.','')
                            petitioner = raw_tag_data_list[0].select('b')[0].get_text(strip=True)
